...
BEGIN
  FOR q IN SELECT id FROM quizzes WHERE title ILIKE '%practice%' LOOP
    ...
    IF EXISTS (SELECT 1 FROM questions WHERE quiz_id = quiz_id AND text = 'Identify the aromatic ring shown (ignore substituents).') THEN
      CONTINUE;
    END IF;

    -- Insert question 1 (with image)
    INSERT INTO questions (quiz_id, position, text, type, points)
    VALUES (quiz_id, (SELECT COALESCE(MAX(position),0)+1 FROM questions WHERE quiz_id = quiz_id), 'Identify the aromatic ring shown (ignore substituents).', 'multiple_choice', 1)
    ...
    -- Insert question 2
    INSERT INTO questions (quiz_id, position, text, type, points)
    VALUES (quiz_id, (SELECT COALESCE(MAX(position),0)+1 FROM questions WHERE quiz_id = quiz_id), 'Which hybridization most describes the carbon atoms in ethene?', 'multiple_choice', 1)
    RETURNING id INTO qid;
    ...
    -- Insert question 3
    INSERT INTO questions (quiz_id, position, text, type, points)
    VALUES (quiz_id, (SELECT COALESCE(MAX(position),0)+1 FROM questions WHERE quiz_id = quiz_id), 'Which statement about electronegativity and bond polarity is TRUE?', 'multiple_choice', 1)
    RETURNING id INTO qid;
